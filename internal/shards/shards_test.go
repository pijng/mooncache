...
func TestDelByHash(t *testing.T) {
	...
	tests := []struct {
		...
		{"item should not be present in cache after delete by hashed key", args{key: "Pelinal Whitestrake is a cyborg proofs", value: value{deep: wantValue}, cost: 0, ttl: math.MaxInt64}, 14388442895772204505, lib.ValueNotPresent()},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_ = Set(tt.args.key, tt.args.value, tt.args.cost, tt.args.ttl)
